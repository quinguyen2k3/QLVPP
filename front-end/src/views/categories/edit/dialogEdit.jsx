...
const EditDialog = ({ categoryId, open, onClose, onSuccess }) => {
  const { t } = useTranslation();
  const { category, loading: loadingGet } = useGetOneCategory(categoryId, open);
  ...
  const formik = useFormik({
    ...
    initialValues: {
      name: category?.name || '',
      note: category?.note || '',
      isActivated: category?.isActivated ?? true,
    },
    validationSchema,
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      <DialogTitle>
        {t('Action.Update')} {t('Menu.Category')}
      </DialogTitle>

...
const PositionListTable = ({ positions = [], loading, onEditClick }) => {
  ...
  const handleDownload = () => {
    const dataToExport = data.map((pos) => ({
      [t('Field.Name')]: pos.name,
      [t('Field.Note')]: pos.note,
      [t('Field.Status')]: pos.isActivated ? t('Status.Active') : t('Status.Inactive'),
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Positions');
    XLSX.writeFile(workbook, 'position-list.xlsx');
  };

  return (
    <DownloadCard title={t('Page.PositionList') || 'Danh sách chức vụ'} onDownload={handleDownload}>

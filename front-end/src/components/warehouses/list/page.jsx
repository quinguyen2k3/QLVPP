...
/* ================= TABLE ================= */
const WarehouseListTable = ({ warehouses = [], loading, onEditClick }) => {
  ...
  const columns = React.useMemo(
    () => [
      columnHelper.accessor((row) => row?.name ?? '', {
        header: t('Field.Name'), // Warehouse Name -> Name
        size: 220,
      }),
  ...
  const handleDownload = () => {
    const dataToExport = warehouses.map((w) => ({
      [t('Field.Name')]: w.name,
      [t('Field.Address')]: w.address,
      [t('Field.Status')]: w.isActivated ? t('Status.Active') : t('Status.Inactive'),
      [t('Field.Date')]: w.createDate,
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Warehouses');
    XLSX.writeFile(workbook, 'warehouse-list.xlsx');
  };

  return (
    <DownloadCard title={t('Page.WarehouseList')} onDownload={handleDownload}>

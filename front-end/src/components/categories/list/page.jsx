...
const CategoryListTable = ({ categories = [], loading, onEditClick }) => {
  ...
  const handleDownload = () => {
    const dataToExport = categories.map((cat) => ({
      [t('Field.Name')]: cat.name,
      [t('Field.Date')]: cat.createDate,
      [t('Field.Status')]: cat.isActivated ? t('Status.Active') : t('Status.Inactive'),
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Categories');
    XLSX.writeFile(workbook, 'category-list.xlsx');
  };

  return (
    <DownloadCard title={t('Page.CategoryList')} onDownload={handleDownload}>

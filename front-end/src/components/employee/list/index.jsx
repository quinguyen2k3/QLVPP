...
const EmployeeListTable = ({ onEditClick }) => {
  const { t } = useTranslation();

  const { employees, warehouses, departments } = useMasterData();
  ...
  const columns = React.useMemo(
    () => [
      columnHelper.accessor((row) => row?.name || 'N/A', {
        header: t('Field.Name'),
        size: 200,
      }),
      columnHelper.accessor((row) => row?.email || 'N/A', {
    ...
    [t, onEditClick, warehouses, departments]
  );
  ...
  const handleDownload = () => {
    const dataToExport = data.map((emp) => {
      const warehouse = warehouses?.find((w) => w.id === emp.warehouseId);
      const department = departments?.find((d) => d.id === emp.departmentId);
      
      return {
        [t('Field.Name')]: emp.name || 'N/A',
        [t('Field.Email')]: emp.email || 'N/A',
        [t('Field.Warehouse')]: warehouse?.name || 'N/A',
        [t('Field.Department')]: department?.name || 'N/A',
      };
    });
  ...
  return (
    <DownloadCard title={t('Page.EmployeeList') || 'Employee List'} onDownload={handleDownload}>

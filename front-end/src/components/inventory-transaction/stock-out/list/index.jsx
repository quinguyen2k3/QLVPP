...
const StockOutList = () => {
  ...
  const { stockOuts, loading, deleteStockOut, fetchAllStockOuts, fetchMyRequests, fetchToApprove } =
  ...
  const filteredStockOuts = stockOuts.filter((item) => {
    ...
    if (startDate || endDate) {
      const itemDate = parseDateFromApi(item.stockOutDate);
  ...
  const isEmpty = filteredStockOuts.length === 0;
  ...
  const Pending = stockOuts.filter(
    (s) => s.status === 'Pending' || s.status === 'PENDING',
  ).length;
  const Approved = stockOuts.filter(
    (s) => s.status === 'Approved' || s.status === 'APPROVED',
  ).length;
  const Received = stockOuts.filter(
    (s) => s.status === 'Received' || s.status === 'RECEIVED',
  ).length;
  const Cancelled = stockOuts.filter(
  ...
  const toggleSelectAll = () => {
    ...
    setSelectAll(value);
    setSelectedItems(value ? filteredStockOuts.map((s) => s.id) : []);
  };
            ...
            >
              <Stack direction="row" gap={2} alignItems="center">
                <Box
                  ...
                  justifyContent="center"
                >
                  <IconTruck color="white" width={22} />
                </Box>
                <Box>
                  <Typography variant="subtitle2" color="textSecondary">
                    {t('Action.Received')}
                  ...
                  {isEmpty ? (
                    ...
                    paginatedStockOuts.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell>{renderDate(item.stockOutDate)}</TableCell>
                        <TableCell>
                          <Chip
                            size="small"
                        ...
                        <TableCell align="center">
                          <Stack direction="row" justifyContent="center" spacing={1}>
                            ...
                            <Tooltip title={t('Action.Edit')}>
                              <IconButton
                                component={Link}
                                to={`/inventory/stock-out/edit/${item.id}`}
                                color="success"
                              >
                                <IconEdit />
                              </IconButton>

...
const UnitListTable = ({ units = [], loading, onEditClick }) => {
  ...
  const handleDownload = () => {
    const dataToExport = units.map((item) => ({
      [t('Field.Name')]: item.name,
      [t('Field.Date')]: item.createDate,
      [t('Field.Status')]: item.isActivated ? t('Status.Active') : t('Status.Inactive'),
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Units');
    XLSX.writeFile(workbook, 'unit-list.xlsx');
  };

  return (
    <DownloadCard title={t('Page.UnitList')} onDownload={handleDownload}>
